...
def test_macd_parameters():
    ...
    print(f"\nDefault Parameters:")
    print(f"  MACD: {macd_default[0]:.4f}, Signal: {macd_default[1]:.4f}, Histogram: {macd_default[2]:.4f}")

    print(f"\nExplicit (12, 26, 9) Parameters:")
    print(f"  MACD: {macd_explicit[0]:.4f}, Signal: {macd_explicit[1]:.4f}, Histogram: {macd_explicit[2]:.4f}")
...
def test_macd_formula():
    ...
    prices = pd.Series([100, 102, 104, 103, 105, 107, 106, 108, 110, 109,
                        ...
                        122, 121, 123, 125, 124, 126, 128] * 2)  # 54 data points
    ...
    print(f"  Match: {'✓' if abs(macd_histogram - float(histogram_manual.iloc[-1])) < 0.0001 else '✗'}")
    ...
    assert abs(macd_histogram - float(histogram_manual.iloc[-1])) < 0.0001, "Histogram calculation mismatch"
