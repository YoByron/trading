...
def test_expiry_is_friday():
    ...
    assert expiry.weekday() == 4, f"Expected Friday (4), got {expiry.weekday()} ({expiry.strftime('%A')})"
...
def test_jan23_2026_bug():
    ...
    assert corrected.weekday() == 4, f"Corrected date should be Friday, got {corrected.strftime('%A')}"
