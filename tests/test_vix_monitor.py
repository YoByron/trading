        ...
        def mock_ticker_side_effect(symbol):
            ...
            if symbol == "^VIX":
                if call_counter['count'] % 3 == 0:
                    ...
                    dates = pd.date_range(end=datetime.now(), periods=252, freq='D')
                    mock.history.return_value = pd.DataFrame({'Close': np.full(252, 16.0)}, index=dates)
                call_counter['count'] += 1
            elif symbol == "^VXV":
                mock.history.return_value = pd.DataFrame({'Close': [20.0]})
            elif symbol == "^VVIX":
...
class TestVIXSignals(unittest.TestCase):
    ...
    def test_get_strategy_recommendation_sell_premium(self):
        ...
        with patch.object(self.signals, 'should_sell_premium') as mock_sell, \
