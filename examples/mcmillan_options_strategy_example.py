...
class McMillanOptionsStrategy:
    ...
    def analyze_options_opportunity(
        ...
        iv_rec = self.kb.get_iv_recommendation(
            iv_rank=iv_rank,
        ...
        logger.info(f"  1σ Move: ${move_1sd['expected_move']} ({move_1sd['probability']*100}% prob)")
        logger.info(f"    Range: ${move_1sd['lower_bound']} - ${move_1sd['upper_bound']}")
        logger.info(f"  2σ Move: ${move_2sd['expected_move']} ({move_2sd['probability']*100}% prob)")
        ...
        if all_checks_passed:
            logger.info(f"TRADE APPROVED: All risk checks passed ✓")
        else:
            logger.info(f"TRADE REJECTED: Some risk checks failed ✗")
        logger.info(f"{'='*80}\n")
...
def example_analysis():
    ...
    print("SUMMARY")
    print("="*80)
    print(f"Example 1 (NVDA): {analysis1['strategy']} - {'APPROVED' if analysis1['trade_approved'] else 'REJECTED'}")
    print(f"Example 2 (AAPL): {analysis2['strategy']} - {'APPROVED' if analysis2['trade_approved'] else 'REJECTED'}")
    print(f"Example 3 (MSFT): {analysis3['strategy']} - {'APPROVED' if analysis3['trade_approved'] else 'REJECTED'}")
    print("="*80 + "\n")
