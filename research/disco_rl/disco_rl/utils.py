...
def gather_from_devices(data: _T) -> _T:
    return jax.tree.map(lambda x: x.reshape((-1, *x.shape[2:])), jax.device_get(data))


def batch_lookup(table: chex.Array, index: chex.Array, num_dims: int = 2) -> chex.Array:
...
def tree_stack(elems: Sequence[chex.ArrayTree], axis: int = 0) -> chex.ArrayTree:
    ...
    if host_data:
        ...
        def conditional_cast(x):
            if isinstance(x, (np.ndarray, jnp.ndarray)):
                if np.issubdtype(x.dtype, np.floating) or jnp.issubdtype(x.dtype, jnp.floating):
...
class MovingAverage:
    ...
    def init_state(self) -> types.EmaState:
        zeros = jax.tree.map(lambda x: jnp.zeros((), jnp.float32), self._example_tree)
    ...
    def _compute_moments(self, state: types.EmaState) -> tuple[chex.ArrayTree, chex.ArrayTree]:
