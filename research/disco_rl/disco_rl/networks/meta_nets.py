...
class LSTM(MetaNet):
    ...
    def __init__(
        ...
        self._meta_rnn_core: MetaLSTM = MetaLSTM(**meta_rnn_kwargs, input_option=input_option)
    ...
    def __call__(
        ...
        meta_input_emb = hk.BatchApply(hk.Linear(1, w_init=self._output_init))(x)  # [T, B, 1]

        # Compute the y, z targets.
        y_hat = hk.BatchApply(hk.Linear(self._prediction_size, w_init=self._aux_init))(x)
        z_hat = hk.BatchApply(hk.Linear(self._prediction_size, w_init=self._aux_init))(x)
        ...
        w = hk.BatchApply(hk.Linear(1, w_init=self._policy_target_init))(w)  # [T, B, A, 1]
...
def _multi_level_extract_by_attr_or_key(x: Any, keys: str) -> Any:
    ...
    for key in keys.split("/"):
        if x is None:
            raise ValueError(
                f"x/{'/'.join(processed_keys)} is `None`, cannot recurse up to x/{keys}"
            )
...
def _construct_input(
    ...
    policy = lax.stop_gradient(jax.nn.softmax(inputs.agent_out["logits"]))  # [T+1, B, A]
    ...
    if input_option.action_conditional:
        ...
        act_cond_inputs.append(
            jnp.expand_dims(jax.nn.one_hot(actions, num_actions, dtype=jnp.float32), axis=-1)
        )
...
def _batch_mlp(hiddens: Sequence[int], num_dims: int = 2) -> Callable[[chex.Array], chex.Array]:
...
def _conv1d_net(channels: Sequence[int]) -> Callable[[chex.Array], chex.Array]:
    return hk.Sequential([functools.partial(_conv1d_block, n_channels=c) for c in channels])
