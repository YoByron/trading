...
def get_value_outs(
    ...
    if len(ema_arg_undef) not in (0, len(ema_arg_names)):
        raise ValueError(f"Received some but not all ema args, undef: {ema_arg_undef}.")
    ...
    if adv_ema_state is not None and adv_ema_fn is not None:
        ...
        normalized_qv_adv = jax.tree.map(
            lambda _, x: adv_ema_fn.normalize(
                x, new_adv_ema_state
    ...
    if td_ema_state is not None and td_ema_fn is not None:
        ...
        if q_net_out is None:
            ...
            normalized_td = td_ema_fn.normalize(tds, new_td_ema_state, subtract_mean=False)
            ...
            normalized_q_td = td_ema_fn.normalize(q_tds, new_td_ema_state, subtract_mean=False)
...
def estimate_values(
    ...
    chex.assert_equal_shape([values[:-1], target_values[:-1], rho, rewards, env_discounts])
...
def estimate_q_values(
    ...
    chex.assert_equal_shape_prefix((values, jax.tree_util.tree_leaves(q_values)[0]), 2)
    q_a = jax.tree.map(lambda x: utils.batch_lookup(x[:-1], actions), q_values)
    target_q_a = jax.tree.map(lambda x: utils.batch_lookup(x[:-1], actions), target_q_values)
    ...
    qv_adv = jax.tree.map(lambda x: x - jnp.expand_dims(target_values, axis=2), target_q_values)
...
def extract_scalar_values_from_net_out(
    ...
    if values is None:
        ...
        values = jax.tree.map(lambda p, q: jnp.sum(p * q, axis=2), pi_tree, q_values)
...
def importance_weight(
    ...
    def log_prob_fn(t, a):
        return distrax.Softmax(t).log_prob(a)
...
def value_logits_to_scalar(value_logits: chex.Array, max_abs_value: float) -> chex.Array:
    """Converts logits to scalar assuming integer bins centred on zero."""
    if max_abs_value <= 0.0:
        raise ValueError(f"Max abs value must be greater than 0: {max_abs_value} <= 0.")
...
def scalar_to_categorical_value(
    ...
    if max_abs_value <= 0.0:
        raise ValueError(f"Maximum abs value must be greater than 0: {max_abs_value} <= 0.")
...
def value_loss_from_target(
    ...
    if categorical_value:
        ...
        value_target_probs = scalar_to_categorical_value(num_bins, value_target, max_abs_value)
        chex.assert_equal_shape([value_net_out, value_target_probs])
        loss_per_step = losses.softmax_cross_entropy(value_net_out, value_target_probs)
    else:
