...
class BatchedSingleStreamEnvironment(base.Environment):
    ...
    def __init__(
        ...
        if "random_seed" in env_settings:
            ...
            seeds = jax.random.randint(jax.random.PRNGKey(seed), (self._num_envs,), 0, max_seed)
            envs = []
            ...
            self._envs = tuple(env_class(env_settings=env_settings) for _ in range(self._num_envs))
    ...
    def step(
        ...
        actions = jax.tree.map(lambda a: a.reshape((self._num_envs,) + a.shape[2:]), actions)
        actions_list = rlax.tree_split_leaves(actions)
        self._states = [env.step(None, a)[1] for env, a in zip(self._envs, actions_list)]
