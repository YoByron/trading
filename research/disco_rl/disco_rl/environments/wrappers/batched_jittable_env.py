...
def _to_env_timestep(
    ...
    return types.EnvironmentTimestep(
        step_type=jax.lax.select(is_terminal, dm_env.StepType.LAST, dm_env.StepType.MID),
...
class BatchedJittableEnvironment(base.Environment):
    ...
    def __init__(
        ...
        self._single_action_spec = dm_specs.BoundedArray((), np.int32, 0, self._env.num_actions - 1)
    ...
    def reset(
        ...
        EnvState, types.EnvironmentTimestep
    ]:  # pytype: disable=signature-mismatch  # numpy-scalars
