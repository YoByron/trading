...
class ActorCritic(base.UpdateRule):
    ...
    def __init__(
        ...
        if normalize_td and categorical_value:
            raise ValueError("normalize_td and categorical_value should not be used together.")
    ...
    def flat_output_spec(self, single_action_spec: types.ActionSpec) -> types.Specs:
        ...
        return dict(
            ...
            v=types.ArraySpec((self._num_bins if self._categorical_value else 1,), np.float32),
        )
    ...
    def agent_loss(
        ...
        pg_advs = meta_out["normalized_advs"] if self._normalize_adv else meta_out["raw_advs"]
        value_tds = meta_out["normalized_tds"] if self._normalize_td else meta_out["tds"]
