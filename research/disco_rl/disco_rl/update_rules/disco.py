...
class DiscoUpdateRule(base.UpdateRule):
    ...
    def init_params(self, rng: chex.PRNGKey) -> tuple[types.MetaParams, chex.ArrayTree]:
        dummy_input = self._get_dummy_input(include_behaviour_out=True, include_agent_adv=True)
        meta_params, meta_rnn_state = self._eta_init_fn(rng, dummy_input, axis_name=None)
    ...
    def flat_output_spec(self, single_action_spec: types.ActionSpec) -> types.Specs:
        return dict(
            logits=utils.get_logits_specs(single_action_spec),
            y=types.ArraySpec((self._prediction_size,), jnp.float32),
        )

    def model_output_spec(self, single_action_spec: types.ActionSpec) -> types.Specs:
        return dict(
    ...
    def unroll_meta_net(
        ...
        chex.assert_tree_shape_prefix((rollout.agent_out, rollout.actions), (t + 1, b))
    ...
    def agent_loss(
        ...
        log = dict(
            ...
            aux_target_entropy=jnp.mean(distrax.Softmax(aux_policy_target).entropy()),
        )
...
def get_input_option() -> types.MetaNetInputOption:
    ...
    return types.MetaNetInputOption(
        base=(
            ...
            types.TransformConfig(source="agent_out/y", transforms=("softmax", "y_net", "td_pair")),
            types.TransformConfig(
