...
def main():
    ...
    if source_data.get('reddit'):
        ...
        for sub in subreddits:
            ...
            source_counts[f"Reddit: r/{sub}"] = reddit_total_posts // len(subreddits) if subreddits else 0
    ...
    if source_counts:
        ...
        with col2:
            ...
            source_df = pd.DataFrame([
                {'Source': k, 'Count': v, 'Percentage': f"{(v/sum(source_counts.values())*100):.1f}%"}
    ...
    if source_data.get('reddit'):
        ...
        for ticker, data in reddit_sentiments.items():
            reddit_data.append({
                ...
                'Sentiment': '🟢 Bullish' if data.get('score', 0) > 50 else '🔴 Bearish' if data.get('score', 0) < -50 else '🟡 Neutral'
            })
    ...
    if source_data.get('news'):
        ...
        for ticker, data in news_sentiments.items():
            ...
            news_data.append({
                ...
                'Sentiment': '🟢 Bullish' if data.get('score', 0) > 20 else '🔴 Bearish' if data.get('score', 0) < -20 else '🟡 Neutral'
            })
    ...
    with col1:
        ...
        coverage = len([t for t in target_tickers if t in covered_tickers]) / len(target_tickers) * 100
