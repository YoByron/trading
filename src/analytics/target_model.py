...
class TargetModel:
    ...
    def calculate(self, params: TargetModelParameters) -> TargetModelOutput:
        ...
        expected_value_per_trade = (
            win_rate * avg_profit_per_winning_trade +
            loss_rate * avg_loss_per_losing_trade
        )
        ...
        max_daily_loss_limit = min(
            params.target_daily_net_income * 3,
            effective_capital * 0.03
        )
    ...
    def _assess_feasibility(
        ...
        if required_annual_return_pct > 100:
            score -= 30
            warnings.append(f"Required annual return {required_annual_return_pct:.1f}% is very high (>100%)")
            is_feasible = False
        elif required_annual_return_pct > 50:
            score -= 20
            warnings.append(f"Required annual return {required_annual_return_pct:.1f}% is high (>50%)")
        elif required_annual_return_pct > 30:
            score -= 10
            warnings.append(f"Required annual return {required_annual_return_pct:.1f}% is ambitious")
        
        # Check Sharpe ratio requirement
        if required_sharpe_ratio > 3.0:
            score -= 25
            warnings.append(f"Required Sharpe {required_sharpe_ratio:.2f} is exceptionally high (>3.0)")
            is_feasible = False
        elif required_sharpe_ratio > 2.0:
            score -= 15
            warnings.append(f"Required Sharpe {required_sharpe_ratio:.2f} is high (>2.0) - difficult to sustain")
        
        # Check trading frequency
        if required_trades_per_day > 10:
            score -= 20
            warnings.append(f"Required {required_trades_per_day:.1f} trades/day is high (>10) - may trigger PDT")
            is_feasible = False
            ...
            score -= 5
            warnings.append(f"Required {required_trades_per_day:.1f} trades/day is very low - may miss opportunities")
        ...
        if capital_per_dollar_target < 50:
            score -= 30
            warnings.append(f"Capital ratio {capital_per_dollar_target:.0f}:1 is very low (<50:1) - high risk")
            is_feasible = False
    ...
    def calculate_progress_to_target(
        ...
        if actual_daily_pnl > 0:
            ...
            days_to_target = "N/A - already at target" if daily_improvement_needed <= 0 else "Unknown"
        else:
...
def generate_target_model_report(
    ...
    if actual_daily_pnl is not None and days_measured is not None:
        ...
        report.append(f"Actual Daily P&L: ${actual_daily_pnl:.2f} ({progress['pnl_progress_pct']:.1f}% of target)")
        report.append(f"Win Rate: {actual_win_rate * 100:.1f}% ({progress['win_rate_progress_pct']:.1f}% of required)")
        report.append(f"Sharpe Ratio: {actual_sharpe:.2f} ({progress['sharpe_progress_pct']:.1f}% of required)")
        report.append(f"% Days ≥ ${target_daily_income}: ~{progress['estimated_pct_days_above_target']:.0f}%")
