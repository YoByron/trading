...
class StrategyRegistry:
    ...
    def update_backtest_metrics(
        self, strategy_id: str, metrics: BacktestMetrics
    ) -> bool:
    ...
    def find_in_development(self) -> list[StrategyRecord]:
        ...
        return [
            s for s in self.strategies.values()
            if s.branch is not None or s.pr_number is not None
        ]
    ...
    def get_best_by_metric(
        ...
        return max(
            candidates,
            key=lambda s: getattr(s.backtest_metrics, metric, 0)
        )
    ...
    def generate_report(self) -> str:
        ...
        if best_sharpe and best_sharpe.backtest_metrics:
            lines.append(f"  • Best Sharpe: {best_sharpe.name} ({best_sharpe.backtest_metrics.sharpe_ratio:.2f})")
        
        best_pnl = self.get_best_by_metric("avg_daily_pnl")
        if best_pnl and best_pnl.backtest_metrics:
            lines.append(f"  • Best Daily P&L: {best_pnl.name} (${best_pnl.backtest_metrics.avg_daily_pnl:.2f}/day)")
        
        best_win_rate = self.get_best_by_metric("win_rate_pct")
        if best_win_rate and best_win_rate.backtest_metrics:
            lines.append(f"  • Best Win Rate: {best_win_rate.name} ({best_win_rate.backtest_metrics.win_rate_pct:.1f}%)")
