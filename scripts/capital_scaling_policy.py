...
class CapitalScalingPolicy:
    ...
    def check_pl_sanity(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"P/L sanity: {'✅ healthy' if is_healthy else '⚠️ alerts detected'}")
    ...
    def calculate_scaling_decision(
        ...
        5. If all pass → approve scaling to min(current * max_scale, recommended)
        6. Otherwise → maintain current budget
        ...
        if not gate_passed:
            ...
            scaling_approved = False
            reasons.append(
                "❌ Promotion gate passed with OVERRIDE flag (manual bypass) - "
                "scaling requires clean pass"
            )

        else:
            reasons.append("✅ Promotion gate passed (all quality thresholds met)")
        ...
        if not sanity_healthy:
            ...
            reasons.append("✅ P/L sanity check passed (no silent failures detected)")
        ...
        if scaling_factor is not None and scaling_factor > self.max_scale_factor:
            scaling_approved = False
            reasons.append(
                f"❌ Scaling factor {scaling_factor:.1f}x exceeds "
            ...
            reasons.append(
                f"✅ Scaling factor {scaling_factor:.1f}x within "
        ...
        if scaling_approved and recommended_budget is not None:
            ...
            reasons.append("")
            reasons.append(
                f"🎯 RECOMMENDATION: Scale from ${current_budget:.2f}/day to "
            ...
            reasons.append("")
            reasons.append(
                f"🛑 RECOMMENDATION: Maintain current budget ${current_budget:.2f}/day "
    ...
    def generate_report(self, decision: ScalingDecision) -> str:
        ...
        if gate["override_active"]:
            lines.append("   ⚠️  Override flag active (manual bypass)")
        if gate["deficit_count"] > 0:
            lines.append(f"   Deficits: {gate['deficit_count']}")
        lines.append("")

        # Sanity status
        lines.append("🔍 P/L SANITY:")
        sanity = decision.sanity_status
        status_emoji = "✅" if sanity["healthy"] else "❌"
