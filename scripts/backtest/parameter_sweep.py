...
from bull_put_spread_backtester import BullPutSpreadBacktester, BacktestConfig


# Parameter grid to test
PARAMETER_GRID = {
    "short_put_delta_min": [-0.70, -0.60, -0.50],
    "short_put_delta_max": [-0.30, -0.25, -0.20],
    "long_put_delta_min": [-0.50, -0.40, -0.35],
    "long_put_delta_max": [-0.30, -0.25, -0.20],
    "spread_width_min": [1.0, 2.0, 3.0],
    "spread_width_max": [3.0, 4.0, 5.0],
    "target_profit_pct": [0.25, 0.50, 0.75],
    "delta_stop_loss_multiplier": [1.5, 2.0, 2.5, 3.0],
}
...
def generate_valid_combinations(grid: Dict) -> List[Dict]:
    ...
    for vals in product(*grid.values()):
        ...
        # Validation rules
        # Short delta range must be valid (min < max, both negative)
        if params["short_put_delta_min"] > params["short_put_delta_max"]:
            continue
        
        # Long delta range must be valid
        if params["long_put_delta_min"] > params["long_put_delta_max"]:
            continue
        
        # Spread width must be valid
        if params["spread_width_min"] > params["spread_width_max"]:
            continue
        
        # Long put should be further OTM (smaller absolute delta)
        if params["long_put_delta_max"] > params["short_put_delta_min"]:
            continue
...
def run_parameter_sweep(
    ...
    for i, params in enumerate(valid_combos[:max_combinations]):
        ...
        try:
            ...
            print(f"  ✅ P&L: ${metrics['total_pnl']:.2f} | Win Rate: {metrics['win_rate']*100:.1f}% | Sharpe: {metrics['sharpe_ratio']:.2f}")
...
def main():
    ...
    parser.add_argument("--output", type=str, default="data/backtests/parameter_sweeps", help="Output directory")
    ...
    for i, row in results_df.head(5).iterrows():
        print(f"\n#{results_df.index.get_loc(i)+1} Sharpe: {row['sharpe_ratio']:.2f} | P&L: ${row['total_pnl']:.2f} | Win Rate: {row['win_rate']*100:.1f}%")
        params = row['params']
