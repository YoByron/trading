...
def main():
    ...
    try:
        ...
        if status['status'] == "complete":
            ...
            print(f"   Days to Target: {status['days_to_target']:.0f} days ({status['days_to_target']/365:.1f} years)")
            ...
            for milestone in milestones:
                if status['progress_pct'] < milestone:
                    ...
                    print(f"   {milestone}% ({shares_at_milestone:.1f} shares): {days:.0f} days (${cost:,.2f})")
        ...
        if status['status'] == "complete" or status['progress_pct'] > 50:
            ...
            position_value = status['position_value'] if status['status'] == "complete" else status['target_shares'] * status['current_price']
