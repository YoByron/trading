...
def check_configuration():
    ...
    print(f"Bond orders will execute: {'✅ YES' if bond_amount >= MIN_THRESHOLD else '❌ NO (too small)'}")
    print(f"REIT orders will execute: {'✅ YES' if reit_amount >= MIN_THRESHOLD else '❌ NO (too small)'}")
    print(f"Treasury orders will execute: {'✅ YES' if treasury_amount >= MIN_THRESHOLD else '❌ NO (too small)'}")
    print()
                ...
                with open(trade_file) as f:
                    ...
                    for trade in trades:
                        ...
                        if trade.get("symbol") in ["BND", "TLT"]:
                            bond_trades += 1
                            print(f"✅ Found bond trade: {trade.get('symbol')} ${trade.get('amount', 0):.2f} on {trade_file.stem}")
    ...
    if bond_amount < MIN_THRESHOLD:
        print(f"⚠️  Bond allocation (${bond_amount:.2f}) is below minimum threshold (${MIN_THRESHOLD:.2f})")
        print(f"   Increase DAILY_INVESTMENT or TIER1_ALLOCATION to enable bond trading")
    elif bond_amount >= MIN_THRESHOLD:
        print(f"✅ Bond allocation (${bond_amount:.2f}) is sufficient for execution")
        print(f"   If bonds aren't executing, check:")
        print(f"   1. Execution logs for errors")
        print(f"   2. Market hours (bonds trade during market hours)")
        print(f"   3. Account buying power")
        print(f"   4. Order validation failures")
    print()
