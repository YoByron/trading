...
def find_potential_conflicts(branch_analysis: dict, strategy_registry: StrategyRegistry) -> list[dict]:
...
def generate_dashboard_report() -> str:
    ...
    if best_sharpe and best_sharpe.backtest_metrics:
        ...
        lines.append(f"   Sharpe: {m.sharpe_ratio:.2f}, Return: {m.total_return_pct:.1f}%, Drawdown: {m.max_drawdown_pct:.1f}%")
    ...
    if strategies_in_dev > 3:
        lines.append("⚠️  Many strategies in development - consider finishing some before starting new ones")
    
    if strategies_backtested > strategies_live + 1:
        lines.append("💡 Multiple backtested strategies ready - consider deploying best one to paper/live")
    
    if best_sharpe and best_sharpe.backtest_metrics:
        if best_sharpe.backtest_metrics.sharpe_ratio > 2.0:
            lines.append(f"✅ {best_sharpe.name} has excellent Sharpe ({best_sharpe.backtest_metrics.sharpe_ratio:.2f}) - strong candidate for live deployment")
