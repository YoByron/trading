...
def generate_dashboard():
    ...
    # Calculate Metrics
    shadow_pl = sum(t.get("amount", 0) * 0.05 for t in analysis_trades)  # Mock P&L calculation (5% random win)
    
    # Today's Snapshot
    today_trades = [t for t in recent_trades if t.get("timestamp", "").startswith(date.today().isoformat())]
    ...
    md = f"""# 🧠 AI Trading Center (2025 Gen)
...
| **Execution Mode** | {"🟡 SHADOW / ANALYSIS" if analysis_trades else "🔴 LIVE TRADING"} | {"Zero Fault Analysis Active" if analysis_trades else "Capital at Risk"} |
...
"""
    ...
    for t in recent_trades[:10]:
        ...
        mode_icon = "🕵️" if mode == "ANALYSIS" else "⚡"
        sym = t.get("symbol")
        act = t.get("action")
        amt = t.get("amount")
