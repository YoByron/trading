#!/bin/bash
# Setup launchd daemon for continuous training monitoring and dashboard updates
# Runs every hour to check training status and update dashboard

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
LAUNCHD_DIR="${HOME}/Library/LaunchAgents"

echo "🔧 Setting up training monitor launchd daemon..."
echo ""

# Create LaunchAgents directory if it doesn't exist
mkdir -p "${LAUNCHD_DIR}"

# Create plist for hourly monitoring
cat > "${LAUNCHD_DIR}/com.trading.training_monitor.plist" <<EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.trading.training_monitor</string>

    <key>ProgramArguments</key>
    <array>
        <string>${REPO_ROOT}/venv/bin/python3</string>
        <string>${REPO_ROOT}/scripts/monitor_training_and_update_dashboard.py</string>
        <string>--once</string>
    </array>

    <key>WorkingDirectory</key>
    <string>${REPO_ROOT}</string>

    <key>StandardOutPath</key>
    <string>${REPO_ROOT}/logs/launchd_training_monitor_stdout.log</string>

    <key>StandardErrorPath</key>
    <string>${REPO_ROOT}/logs/launchd_training_monitor_stderr.log</string>

    <key>RunAtLoad</key>
    <false/>

    <key>KeepAlive</key>
    <false/>

    <key>EnvironmentVariables</key>
    <dict>
        <key>PATH</key>
        <string>/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:${HOME}/.local/bin</string>
        <key>HOME</key>
        <string>${HOME}</string>
    </dict>

    <key>StartCalendarInterval</key>
    <array>
        <!-- Every hour at :00 minutes -->
        <dict>
            <key>Minute</key>
            <integer>0</integer>
        </dict>
    </array>
</dict>
</plist>
EOF

echo "✅ Created ${LAUNCHD_DIR}/com.trading.training_monitor.plist"
echo ""
echo "📋 Next steps:"
echo "   1. Load the daemon:"
echo "      launchctl load ${LAUNCHD_DIR}/com.trading.training_monitor.plist"
echo ""
echo "   2. Check status:"
echo "      launchctl list | grep training_monitor"
echo ""
echo "   3. View logs:"
echo "      tail -f ${REPO_ROOT}/logs/launchd_training_monitor_*.log"
echo ""
echo "   4. Test manually:"
echo "      python3 ${REPO_ROOT}/scripts/monitor_training_and_update_dashboard.py --once"
echo ""
echo "🎯 This daemon runs hourly to monitor training and update the Progress Dashboard"
