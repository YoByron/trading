...
def analyze_options_path(
    ...
    results = {}
    target_daily_investment = (target_months * 30 * current_daily_investment) / (
        target_months * 30
    )  # Placeholder
    ...
    for symbol in symbols:
        try:
            ...
            print(f"   Days at ${current_daily_investment:.2f}/day: {days_at_current:.0f} ({days_at_current/365:.1f} years)")
...
def main():
    ...
    print("=" * 80)
    analyze_options_path(
        symbols, current_daily_investment=50.0, target_shares=50, target_months=8
    )
